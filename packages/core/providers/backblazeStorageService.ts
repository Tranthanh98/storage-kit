...
/**
 * Storage service implementation for Backblaze B2 (S3-compatible API).
 *
 * Backblaze B2 is a cloud storage service that provides an S3-compatible API.
...
export class BackBlazeStorageService implements IStorageService {
  ...
  constructor(config?: Partial<StorageConfig>) {
    this.config = {
      ...
      secretAccessKey: config?.secretAccessKey ?? process.env.BACKBLAZE_APPLICATION_KEY ?? "",
      region: config?.region ?? process.env.BACKBLAZE_REGION ?? "",
      publicUrlBase: config?.publicUrlBase ?? process.env.BACKBLAZE_PUBLIC_URL,
  ...
  private getPublicUrl(key: string): string {
    ...
    // Default Backblaze B2 URL pattern
